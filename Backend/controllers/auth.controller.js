...
const registerUser = asyncHandler(async(req,res,next)=>{
        ...
        return res.status(200).json(new ApiResponse(200,"User Reistered Successfully"))
}
...
const loginUser = asyncHandler(async(req,res,next)=>{
    ...
    return res
        .status(200)
        .cookie("accessToken",accessToken, {httpOnly:true ,secure:true ,path:'/',maxAge:process.env.ACCESS_TOKEN_EXPIRY,sameSite: 'None',domain: process.env.CLIENT_URL,path:"/" }) 
        .cookie("refreshToken",refreshToken,{httpOnly:true ,secure:true ,path:'/',maxAge:process.env.REFRESH_TOKEN_EXPIRY,sameSite: 'None',domain: process.env.CLIENT_URL,path:"/"})  
        .cookie("loggedInUserInfo", JSON.stringify({role:loggedInUser.role , userId: loggedInUser._id}),{httpOnly:false ,path:'/',secure:true ,maxAge:process.env.REFRESH_TOKEN_EXPIRY,sameSite: 'None',domain: process.env.CLIENT_URL,path:"/" })
...
const resetPassword =  asyncHandler(async(req,res,next)=>{
        ...
        const fetchedUser = await User.findOne({email:email , resetPasswordToken : token})
