...
export function AdminCreateProductForm() {
  ...
  const validations = {
    ...
    price:{
      ...
      validate: {
        isDecimal: (value) => !isNaN(value) || "Enter a valid price value (e.g., 120.00)",
    ...
    discountPercentage:{
      ...
      validate: {
        isDecimal: (value) => !isNaN(value) || "Enter a valid value (e.g., 12.34)",
      },
    },
    description:{
    ...
    variations: {
      validate: {
        atLeastOne: (value) => value.length > 0 ? true : "At least one variation is required",
      },
    },
        ...
        <form className="mt-5" onSubmit={handleSubmit(formSubmitHandler)}>
          ...
          <div className="wrapper flex flex-col sm:flex-row sm:gap-10">
            ...
            <div className="discountPercentage mb-8 max-w-[400px]  flex-col sm:justify-center gap-2">
              ...
              <input
                type="decimal"
                {...register("discountPercentage",{...validations.discountPercentage})}
                className="focus:border-indigo-600 px-2 mt-2 w-full h-9 bg-[#FFFFFF] rounded-md border-2 border-[#D1D5DB] outline-none"
              />
              </div>
