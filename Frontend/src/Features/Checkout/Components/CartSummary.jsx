...
export function CartSummary({ selectedAddressIndex, paymentMethod, watch }) {
  ...
  function createOrderHandler() {
    ...
    console.log({ selectedAddressIndex, paymentMethod, email, phoneNumber, fullName });
    const orderDetails = {
    ...
    <div className="wrapper w-full sm:w-[90%] p-3 sm:p-10 box-border mx-auto my-10 bg-white flex flex-col gap-4">
      ...
      <div className="cart-items-wrapper flex flex-col gap-3">
        {cartState?.cartItems?.length === 0 ? (
          ...
          cartState?.cartItems?.map((item) => <ProductTile key={uuid()} {...item} />)
        )}
      </div>
      ...
      {cartState?.cartItems?.length > 0 && (
        <div className="totalPrice flex flex-col gap-3">
          ...
          <div className="subTotal flex justify-between items-center">
            ...
            <span className="font-semibold text-base">
              ...
              {cartState?.cartItems?.reduce((acc, curr) => {
                return (acc +=
                  Math.floor(((100 - curr.product.discountPercentage) / 100) * curr.product.price) * curr.quantity);
              }, 0)}
            </span>
          </div>
          <div className="subTotal flex justify-between items-center">
          ...
          <div className="text-[#6B7280] text-sm">Shipping and taxes calculated at checkout</div>
          ...
          <div className="flex items-center justify-center">
            <span className="text-center text-sm text-gray-500 hover:cursor-pointer">or</span>
