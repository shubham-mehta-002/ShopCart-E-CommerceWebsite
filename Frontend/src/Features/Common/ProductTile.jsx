...
import {addItemToCartAsync , reduceCartItemQuantityAsync ,removeCartItemAsync} from '../Cart/CartSlice'

export function ProductTile({ color,colorCode , product, quantity ,size }) {
  const {_id,thumbnail,price,title,description,discountPercentage} = product
          ...
          <div className="details w-[calc(100%-100px)] flex flex-col gap-1 ml-3">
            ...
            <div className="flex flex-row justify-between items-center">
              <div className="flex flex-row gap-1 items-center ">
                <label htmlFor="quantity" className="mr-1">Qty</label>
                <button 
                ...
                {quantity}
                <button 
                  className={`border-2 p-3 py-0.5 ${quantity===5 ? "cursor-not-allowed" : "cursor-pointer"}`}
